...
class ExperimentVideoCube(object):
    ...
    def __init__(self, root_dir, save_dir, subset, repetition, version, trackers_pred_path):
    ...
    def report(self, tracker_names, attribute_name):
        ...
        for name in tracker_names:
            ...
            for s in range(len(self.dataset.seq_names)):
                ...
                # read absent info
                absent_path = os.path.join(self.root_dir, 'VideoCube-Info', 'attribute', 'absent', '{}.txt'.format(num))
                absent = pd.read_csv(absent_path, header=None, names=['absent'])

                # read shotcut info
                shotcut_path = os.path.join(self.root_dir, 'VideoCube-Info', 'attribute', 'shotcut', '{}.txt'.format(num))
                shotcut = pd.read_csv(shotcut_path, header=None, names=['shotcut'])

                # read attribute info
                if attribute_name != 'normal':
                    attribute_path = os.path.join(self.root_dir, 'VideoCube-Info', 'attribute', attribute_name, '{}.txt'.format(num))
                ...
                res_path = os.path.join(self.trackers_pred_path[name], f"videocube_{self.subset}", f"{num}.txt")
                boxes = np.loadtxt(res_path)
    ...
    def report_robust(self, tracker_names):
        ...
        for name in tracker_names:
            ...
            for s in range(len(self.dataset.seq_names)):
                ...
                corrcoef_path = os.path.join(self.root_dir, 'VideoCube-Info','attribute', 'corrcoef', '{}.txt'.format(num))
                corrcoef = pd.read_csv(corrcoef_path, header=None, names=['corrcoef'])
                corrcoef_mean = np.nanmean(corrcoef)

                restart_path = os.path.join(self.root_dir, 'VideoCube-Info','attribute', 'restart', '{}.txt'.format(num))
                restart = len(open(restart_path, 'r').readlines()) if os.path.getsize(restart_path) != 0 else 1

                # init_path = os.path.join(self.trackers_pred_path[name], f"videocube_{self.subset}", f"{num}_init.txt")
                init_path = os.path.join(self.trackers_pred_path[name], f"init_{name}_restart_{num}_1.txt")
