...
def norm_center_consistency(gts,rects1,rects2,bound):
    threshold = 0.05
    npre_1 = np.zeros(len(rects1))
    npre_2 = np.zeros(len(rects1))
    npre = np.zeros(len(rects1))
    flags = np.zeros(len(rects1))
    #print("#")
    npre_1,flags = normalized_center_error(rects1,gts,bound)
    npre_2,flags = normalized_center_error(rects2,gts,bound)
    npre,flags = normalized_center_error(rects1,rects2,bound)
    #print("#")
    #print(npre)
    single_consistency = np.zeros(len(rects1)+5) # 5: c_exp1,c_exp2,c_obs,c_exp,kappa
    rects1_succ = 0
    rects2_succ = 0
    i = 0
    while i < len(rects1):
        if(npre_1[i] < threshold):
            rects1_succ += 1
        if(npre_2[i] < threshold):
            rects2_succ += 1
        if(npre[i] < threshold):
            single_consistency[i] = 1
            single_consistency[-1] += 1
        i += 1
    c_exp1 = rects1_succ / len(rects1)
    #print("c_exp1",c_exp1)
    c_exp2 = rects2_succ / len(rects1)
    #print("c_exp2",c_exp2)
    c_exp = c_exp1 * c_exp2 + (1 - c_exp1) * (1 - c_exp2)
    c_obs = single_consistency[-1] / len(rects1)
    #print("c_obs",c_obs)
    kappa = (c_obs - c_exp) / (1 - c_exp + 0.0001)
    # print(c_exp1)
    # print(c_exp2)
    if(kappa != 0):
        single_consistency[-1] = kappa
        single_consistency[-2] = c_exp
        single_consistency[-3] = c_obs
        single_consistency[-4] = c_exp2
        single_consistency[-5] = c_exp1
    else:                                    #kappa = 0 的特殊情况
        single_consistency[-1] = 404
        single_consistency[-2] = 404
        single_consistency[-3] = 404
        single_consistency[-4] = 404
        single_consistency[-5] = 404
    return single_consistency

###
#是否同时落入或不落入gts框
def gts_consistency(gts,rects1,rects2):
    single_consistency = np.zeros(len(rects1)+5) # 5: c_exp1,c_exp2,c_obs,c_exp,kappa
    center1_in = np.zeros(len(rects1) + 5)
    center2_in = np.zeros(len(rects1) + 5)
    if rects1.shape[1] == 4:
    ...
    i = 0
    while i < len(rects1):
        if(centers1[i,0]>gts[i,0] and centers1[i,0]<gts[i,0]+gts[i,2] and centers1[i,1]>gts[i,1] and centers1[i,1]<gts[i,1]+gts[i,3]):
            center1_in[i] = 1
            center1_in[-1] += 1
        else:
            center1_in[i] = 0

        if(centers2[i,0]>gts[i,0] and centers2[i,0]<gts[i,0]+gts[i,2] and centers2[i,1]>gts[i,1] and centers2[i,1]<gts[i,1]+gts[i,3]):
            center2_in[i] = 1
            center2_in[-1] += 1
        else:
            center2_in[i] = 0

        if(center1_in[i] + center2_in[i] == 1):
            single_consistency[i] = 0
        else:
            single_consistency[i] = 1
            single_consistency[-1] += 1
        i = i + 1
    c_exp1 = center1_in[-1] / len(rects1)
    # print("c_exp1",c_exp1)
    c_exp2 = center2_in[-1] / len(rects1)
    # print("c_exp2",c_exp2)
    c_exp = c_exp1 * c_exp2 + (1 - c_exp1) * (1 - c_exp2)
    c_obs = single_consistency[-1] / len(rects1)
    # print("c_obs",c_obs)
    # print("c_exp",c_exp)
    kappa = (c_obs - c_exp + 1e-16) / (1 - c_exp + 1e-16)
...
#是否同时落入或不落入gts框
def auc_gts_consistency(gts,rects1,rects2):
    single_consistency = np.zeros(len(rects1)+5) # 5: c_exp1,c_exp2,c_obs,c_exp,kappa
    center1_in = np.zeros(len(rects1) + 5)
    center2_in = np.zeros(len(rects1) + 5)
    if rects1.shape[1] == 4:
        ...
        # iou > 0.5 / iou < 0.5
        center1_in[:len(rects1)] = ious > 0.5
        center1_in[-1] = np.sum(center1_in[:len(rects1)])
    ...
    if rects2.shape[1] == 4:
        ...
        # iou > 0.5 / iou < 0.5
        center2_in[:len(rects2)] = ious > 0.5
        center2_in[-1] = np.sum(center2_in[:len(rects2)])
    else:
        centers2 = rects2
    ...
    for i in range(0, len(rects1)):
        if(center1_in[i] + center2_in[i] == 1):
            ...
            single_consistency[i] = 1
        single_consistency[-1] += single_consistency[i]
...
def error_consistency_center(rects1,rects2):
    if rects1.shape[1] == 4:
        centers1 = rects1[..., :2] + (rects1[..., 2:] - 1) / 2
    else:
        centers1 = rects1
...
def center_bound_error(rects1, rects2,bound):
    r"""Center error.
    OTB-2015
    ...
    if rects2.shape[1] == 4:
        ...
        centers2 = rects2[..., :2] 
    errors = np.sqrt(np.sum(np.power(centers1 - centers2, 2), axis=-1)) / bound * 100
