...
class VideoCube(object):
    ...
    def __init__(self, root_dir, subset, version):
        ...
        if subset in ['train','val','test']:
            self.seq_dirs = [os.path.join(root_dir, subset, s,'frame_{}'.format(s)) for s in self.seq_names]
            self.anno_files = [os.path.join(root_dir, 'VideoCube-Info', 'attribute', 'groundtruth','{}.txt'.format(s)) for s in self.seq_names]
            self.restart_files = [os.path.join(root_dir, 'VideoCube-Info', 'attribute', 'restart','{}.txt'.format(s)) for s in self.seq_names]
